...
class PengajuanPengurusanSuratTanah extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  PengajuanPengurusanSuratTanah(
      {Key? key,
  ...
  @override
  State<PengajuanPengurusanSuratTanah> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<PengajuanPengurusanSuratTanah> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSPPT == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi SPPT PBB",
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratTanah == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi surat tanah",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerrySuratTanah() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratTanah = File(imageFile!.path);
    });
  }

  Future getImageGalerrySPPT() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSPPT = File(imageFile!.path);
    });
  }
                    ...
                    child: imageSuratTanah == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratTanah!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSPPT != null) {
                          final spptFile = await http.MultipartFile.fromPath(
                            'image_sppt',
                            imageSPPT!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSPPT!.path)),
                          );
                          request.files.add(spptFile);
                        }

                        if (imageSuratTanah != null) {
                          final surattanahFile =
                              await http.MultipartFile.fromPath(
                            'image_surattanah',
                            imageSuratTanah!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSuratTanah!.path)),
                          );
                          request.files.add(surattanahFile);
                        }
  ...
  Future<void> uploadDataAndImages() async {
    ...
    if (imageSPPT != null) {
      final spptFile = await http.MultipartFile.fromPath(
        'image_sppt',
        imageSPPT!.path,
        contentType: MediaType('image', path.extension(imageSPPT!.path)),
      );
      request.files.add(spptFile);
    }

    if (imageSuratTanah != null) {
      final surattanahFile = await http.MultipartFile.fromPath(
        'image_surattanah',
        imageSuratTanah!.path,
        contentType: MediaType('image', path.extension(imageSuratTanah!.path)),
      );
      request.files.add(surattanahFile);
    }
