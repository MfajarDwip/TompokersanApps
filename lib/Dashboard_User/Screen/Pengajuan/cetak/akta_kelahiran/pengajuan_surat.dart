...
class Pengajuansurataktalahir extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  Pengajuansurataktalahir(
      {Key? key,
  ...
  @override
  State<Pengajuansurataktalahir> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<Pengajuansurataktalahir> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratNikah == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi surat nikah",
          backgroundColor: black.withOpacity(0.7));
    } else if (imageBidan == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah Surat Bidan Asli",
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratLahir == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah surat lahir dari kelurahan",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerrySuratNikah() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratNikah = File(imageFile!.path);
    });
  }

  Future getImageGalerryBidan() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageBidan = File(imageFile!.path);
    });
  }

  Future getImageGalerrySuratLahir() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratLahir = File(imageFile!.path);
    });
  }
  ...
  File? imageKK;
  File? imageKTP;
  File? imageSuratNikah;
  File? imageBidan;
  File? imageSuratLahir;
  @override
                    ...
                    child: imageSuratNikah == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratNikah!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                    ...
                    child: imageSuratLahir == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratLahir!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSuratNikah != null) {
                          final suratnikahFile =
                              await http.MultipartFile.fromPath(
                            'image_suratnikah',
                            imageSuratNikah!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSuratNikah!.path)),
                          );
                          request.files.add(suratnikahFile);
                        }

                        if (imageBidan != null) {
                          final suratbidanFile =
                              await http.MultipartFile.fromPath(
                            'image_bidan',
                            imageBidan!.path,
                            contentType: MediaType(
                                'image', path.extension(imageBidan!.path)),
                          );
                          request.files.add(suratbidanFile);
                        }

                        if (imageSuratLahir != null) {
                          final suratlahirFile =
                              await http.MultipartFile.fromPath(
                            'image_suratlahir',
                            imageSuratLahir!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSuratLahir!.path)),
                          );
                          request.files.add(suratlahirFile);
                        }
...
// Fungsi untuk mengunggah data dan gambar ke backend (menggunakan paket http)
  Future<void> uploadDataAndImages() async {
    ...
    if (imageSuratNikah != null) {
      final suratnikahFile = await http.MultipartFile.fromPath(
        'image_suratnikah',
        imageSuratNikah!.path,
        contentType: MediaType('image', path.extension(imageSuratNikah!.path)),
      );
      request.files.add(suratnikahFile);
    }

    if (imageBidan != null) {
      final suratbidanFile = await http.MultipartFile.fromPath(
        'image_bidan',
        imageBidan!.path,
        contentType: MediaType('image', path.extension(imageBidan!.path)),
      );
      request.files.add(suratbidanFile);
    }

    if (imageSuratLahir != null) {
      final suratlahirFile = await http.MultipartFile.fromPath(
        'image_suratlahir',
        imageSuratLahir!.path,
        contentType: MediaType('image', path.extension(imageSuratLahir!.path)),
      );
      request.files.add(suratlahirFile);
    }
