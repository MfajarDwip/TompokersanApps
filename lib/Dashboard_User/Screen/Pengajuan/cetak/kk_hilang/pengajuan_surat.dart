...
class Pengajuansuratkkhilang extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  Pengajuansuratkkhilang(
      {Key? key,
  ...
  @override
  State<Pengajuansuratkkhilang> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<Pengajuansuratkkhilang> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratKehilangan == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah surat kehilangan",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerrySuratKehilangan() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratKehilangan = File(imageFile!.path);
    });
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () {
                getImageGalerrySuratKehilangan();
              },
              child: Padding(
                padding: EdgeInsets.symmetric(horizontal: 0, vertical: 20),
                child: DottedBorder(
                    ...
                    strokeCap: StrokeCap.round,
                    color: black,
                    child: imageSuratKehilangan == null
                        ? Container(
                            height: 200,
                            width: double.infinity,
                            decoration: BoxDecoration(
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratKehilangan!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSuratKehilangan != null) {
                          final suratkehilhanganFile = await http.MultipartFile.fromPath(
                            'image_suratkehilangan',
                            imageSuratKehilangan!.path,
                            contentType: MediaType('image',
                                path.extension(imageSuratKehilangan!.path)),
                          );
                          request.files.add(suratkehilhanganFile);
                        }
...
// Fungsi untuk mengunggah data dan gambar ke backend (menggunakan paket http)
  Future<void> uploadDataAndImages() async {
    ...
    if (imageSuratKehilangan != null) {
      final suratkehilanganFile = await http.MultipartFile.fromPath(
        'image_suratkehilangan',
        imageSuratKehilangan!.path,
        contentType: MediaType('image', path.extension(imageSuratKehilangan!.path)),
      );
      request.files.add(suratkehilanganFile);
    }
