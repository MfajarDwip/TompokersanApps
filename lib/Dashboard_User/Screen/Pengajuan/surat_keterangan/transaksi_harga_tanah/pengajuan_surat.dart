...
class PengajuanSuratTanah extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  ...
  @override
  State<PengajuanSuratTanah> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<PengajuanSuratTanah> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSertifikat == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah sertifikat / akta jual beli",
          backgroundColor: black.withOpacity(0.7));
  ...
  Future getImageGalerrySertifikat() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSertifikat = File(imageFile!.path);
    });
  }
                    ...
                    child: imageSertifikat == null
                        ? Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  "Unggah Sertifikat / akta jual beli",
                                  style: MyFont.poppins(
                                      fontSize: 12, color: softgrey),
                                )
                              ],
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSertifikat!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSertifikat != null) {
                          final sertifikatFile =
                              await http.MultipartFile.fromPath(
                            'image_sertifikat',
                            imageSertifikat!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSertifikat!.path)),
                          );
                          request.files.add(sertifikatFile);
                        }
...
// Fungsi untuk mengunggah data dan gambar ke backend (menggunakan paket http)
  Future<void> uploadDataAndImages() async {
    ...
    if (imageSertifikat != null) {
      final sertifikatFile = await http.MultipartFile.fromPath(
        'image_sertifikat',
        imageSertifikat!.path,
        contentType: MediaType('image', path.extension(imageSertifikat!.path)),
      );
      request.files.add(sertifikatFile);
    }
