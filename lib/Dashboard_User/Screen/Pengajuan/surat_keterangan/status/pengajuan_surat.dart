...
class Pengajuansuratstatus extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  Pengajuansuratstatus(
      {Key? key,
  ...
  @override
  State<Pengajuansuratstatus> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<Pengajuansuratstatus> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratKematian == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah surat kematian",
          backgroundColor: black.withOpacity(0.7));
    } else if (imageAktaCerai == null) {
      Fluttertoast.showToast(
          msg: "Silahkan Unggah fotokopi Akta Cerai",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerryAktaCerai() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageAktaCerai = File(imageFile!.path);
    });
  }

  Future getImageGalerrySuratKematian() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratKematian = File(imageFile!.path);
    });
  }
                    ...
                    child: imageAktaCerai == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageAktaCerai!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                    ...
                    child: imageSuratKematian == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratKematian!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageAktaCerai != null) {
                          final aktaceraiFile =
                              await http.MultipartFile.fromPath(
                            'image_aktacerai',
                            imageAktaCerai!.path,
                            contentType: MediaType(
                                'image', path.extension(imageAktaCerai!.path)),
                          );
                          request.files.add(aktaceraiFile);
                        }

                        if (imageSuratKematian != null) {
                          final suratkematianFile =
                              await http.MultipartFile.fromPath(
                            'image_suratkematian',
                            imageSuratKematian!.path,
                            contentType: MediaType('image',
                                path.extension(imageSuratKematian!.path)),
                          );
                          request.files.add(suratkematianFile);
                        }
...
// Fungsi untuk mengunggah data dan gambar ke backend (menggunakan paket http)
  Future<void> uploadDataAndImages() async {
    ...
    if (imageAktaCerai != null) {
      final aktaceraiFile = await http.MultipartFile.fromPath(
        'image_aktacerai',
        imageAktaCerai!.path,
        contentType: MediaType('image', path.extension(imageAktaCerai!.path)),
      );
      request.files.add(aktaceraiFile);
    }

    if (imageSuratKematian != null) {
      final suratkematianFile = await http.MultipartFile.fromPath(
        'image_suratkematian',
        imageSuratKematian!.path,
        contentType:
            MediaType('image', path.extension(imageSuratKematian!.path)),
      );
      request.files.add(suratkematianFile);
    }
