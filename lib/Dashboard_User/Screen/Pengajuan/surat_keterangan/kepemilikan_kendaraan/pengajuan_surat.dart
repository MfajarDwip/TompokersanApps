...
class PengajuanSuratKepemilikanKendaraan extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  PengajuanSuratKepemilikanKendaraan(
      {Key? key,
  ...
  @override
  State<PengajuanSuratKepemilikanKendaraan> createState() =>
      _PengajuansuratState();
}

class _PengajuansuratState extends State<PengajuanSuratKepemilikanKendaraan> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSTNK == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi STNK",
          backgroundColor: black.withOpacity(0.7));
    } else if (imageBPKB == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi BPKB",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerrySTNK() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSTNK = File(imageFile!.path);
    });
  }

  Future getImageGalerryBPKB() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageBPKB = File(imageFile!.path);
    });
  }
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSTNK != null) {
                          final stnkFile = await http.MultipartFile.fromPath(
                            'image_stnk',
                            imageSTNK!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSTNK!.path)),
                          );
                          request.files.add(stnkFile);
                        }

                        if (imageBPKB != null) {
                          final bpkbFile = await http.MultipartFile.fromPath(
                            'image_bpkb',
                            imageBPKB!.path,
                            contentType: MediaType(
                                'image', path.extension(imageBPKB!.path)),
                          );
                          request.files.add(bpkbFile);
                        }
  ...
  Future<void> uploadDataAndImages() async {
    ...
    if (imageSTNK != null) {
      final stnkFile = await http.MultipartFile.fromPath(
        'image_stnk',
        imageSTNK!.path,
        contentType: MediaType('image', path.extension(imageSTNK!.path)),
      );
      request.files.add(stnkFile);
    }
    ...
    if (imageBPKB != null) {
      final bpkbFile = await http.MultipartFile.fromPath(
        'image_bpkb',
        imageBPKB!.path,
        contentType: MediaType('image', path.extension(imageBPKB!.path)),
      );
      request.files.add(bpkbFile);
    }
