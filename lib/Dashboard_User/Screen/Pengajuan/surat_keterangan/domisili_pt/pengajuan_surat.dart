...
class PengajuanSuratDomisilipt extends StatefulWidget {
  String idsurat;
  String namaSurat;
  Keluarga keluarga;
  Masyarakat masyarakat;
  PengajuanSuratDomisilipt(
      {Key? key,
  ...
  @override
  State<PengajuanSuratDomisilipt> createState() => _PengajuansuratState();
}

class _PengajuansuratState extends State<PengajuanSuratDomisilipt> {
  @override
  void initState() {
  ...
  void verifypengajuan() {
    if (keperluan.text.isEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          backgroundColor: black.withOpacity(0.7));
    } else if (imageSuratIzin == null) {
      Fluttertoast.showToast(
          msg: "Silahkan unggah fotokopi surat izin dari yang berwenang",
          backgroundColor: black.withOpacity(0.7));
    } else {
  ...
  Future getImageGalerrySuratIzin() async {
    final picker = ImagePicker();
    final imageFile = await picker.pickImage(source: ImageSource.gallery);
    setState(() {
      imageSuratIzin = File(imageFile!.path);
    });
  }
                    ...
                    child: imageSuratIzin == null
                        ...
                        : Container(
                            ...
                            decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                image: DecorationImage(
                                    image: FileImage(imageSuratIzin!),
                                    fit: BoxFit.cover)),
                          )),
              ),
            ),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      if (keperluan.text.isEmpty) {
                        ...
                        if (imageSuratIzin != null) {
                          final suratizinFile = await http.MultipartFile.fromPath(
                            'image_suratizin',
                            imageSuratIzin!.path,
                            contentType: MediaType(
                                'image', path.extension(imageSuratIzin!.path)),
                          );
                          request.files.add(suratizinFile);
                        }
  ...
  Future<void> uploadDataAndImages() async {
    ...
    if (imageKTP != null) {
      ...
      final suratizinFile = await http.MultipartFile.fromPath(
        'image_suratizin',
        imageSuratIzin!.path,
        contentType: MediaType('image', path.extension(imageSuratIzin!.path)),
      );
      request.files.add(suratizinFile);
    }
